/**
 * SERVICE WORKER — HEALTHCARE ROSTER AVAILABILITY FORM
...
const CACHE_STATIC   = `static-v${VERSION}-healthcare`;
const CACHE_FONTS    = `font-v${VERSION}-healthcare`;
const CACHE_EXTERNAL = `external-v${VERSION}-healthcare`;
...
self.addEventListener('install', (event) => {
  console.log(`[SW] Installing Healthcare service worker v${VERSION}`);
...
self.addEventListener('activate', (event) => {
  console.log('[SW] Activating Healthcare service worker');

  event.waitUntil(
    caches.keys().then((cacheNames) => {
      return Promise.all(
        cacheNames.map((cacheName) => {
          if (!cacheName.includes(`v${VERSION}-healthcare`)) {
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    const title = data.title || 'Healthcare Roster';
    const options = {
      body: data.body || 'You have a new roster message from your coordinator',
  ...
  event.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clientList) => {
      for (const client of clientList) {
        if (client.url.includes('/healthcare-roster-pwa/') && 'focus' in client) {
          return client.focus();
        }
      }
...
console.log(`[SW] Healthcare service worker v${VERSION} loaded`);
