(() => {
  ...
  function buildAvailabilityTable() {
    ...
    DAYS.forEach(day => {
      ...
      // Start cell — default 07:00 (shift start)
      const startCell  = document.createElement('td');
      const startRow   = document.createElement('div');
      startRow.className = 'sc-time-cell';
      startRow.id = 'sc-srow-' + day.key;
      startRow.appendChild(buildTimeSelect('sc-start-' + day.key, '07:00'));
      ...
      // End cell — default 19:00 (shift end)
      ...
      endRow.appendChild(buildTimeSelect('sc-end-' + day.key, '19:00'));
  ...
  function validate() {
    let ok = true;
    const checks = [
      { id: 'sc-manager-email', errId: 'sc-manager-email-err', msg: "Please enter the coordinator's email", test: isValidEmail },
  ...
  function buildPayload() {
    ...
    return {
      ...
      ward:           sanitize($('sc-ward').value, 80),
      availability:   av
    };
  }
  ...
  function makeFilename(name) {
    const slug = (name || 'staff-member')
      .replace(/\s+/g, '-').toLowerCase().replace(/[^a-z0-9-]/g, '');
    return 'health-avail-' + slug + '-' + new Date().toISOString().slice(0, 10) + '.json';
  }
  ...
  function openEmail(managerEmail, memberName, filename) {
    const subj = encodeURIComponent('Healthcare Roster Availability — ' + memberName);
    const body = encodeURIComponent(
      'Hi,\n\nPlease find my weekly availability submission attached as:\n    ' + filename +
      '\n\nTo import it into the Healthcare Roster, go to:\n    Backup → Import Availability → select the attached file\n\n' +
      'Submitted by: '  + memberName + '\nSubmitted at: ' + new Date().toLocaleString() +
      '\n\n— Sent via Healthcare Roster Availability Form'
    );
  ...
  async function handleShare() {
    ...
    try {
      await navigator.share({
        title: 'Healthcare Roster Availability',
        text:  'Weekly availability for ' + (payload.memberName || 'staff member'),
        files: [file]
      });
